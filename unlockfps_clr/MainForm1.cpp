...
#define MUTEX_BUSY "ThreadBusying"

unsigned int g_ThreadLv = 1;//0,1,2,3
...
int main(array<String^>^ args)
{
	...
	HANDLE hMutex_LV = OpenMutexA(MUTEX_ALL_ACCESS, FALSE, MUTEX_BUSY);
	if (hMutex_LV)
		return 0;
	hMutex_LV = CreateMutexA(nullptr, FALSE, MUTEX_BUSY);
...
namespace unlockfpsclr
{
	...
	Void MainForm::OnDoWork(Object^ sender, DoWorkEventArgs^ e)
	{
		...
		while (!worker->CancellationPending)
		{
			...
			if (this->btnAutoSave->Checked)
				worker->ReportProgress(150);
			if (this->cbxQuickSwitch->Checked)
				worker->ReportProgress(151);
		}
	...
	Void MainForm::OnProgressChanged(Object^ sender, ProgressChangedEventArgs^ e)
	{
		...
		WaitForSingleObject(MUTEX_BUSY, INFINITE);
		...
		if (progress == 151 && g_ThreadLv < 1)
		{
			doQuickSwitch();
			g_ThreadLv = 1;
		}

		ReleaseMutex(MUTEX_BUSY);  // 触发互斥量
	}
	...
	System::Void MainForm::btnClearAccount_Click(System::Object^ sender, System::EventArgs^ e)
	{
		...
		if (rk&&rk->OpenSubKey("原神")) {
			rk->DeleteSubKeyTree("原神");
			rk->Close();
			MessageBox::Show("Clear Account Successfully!", "Info", MessageBoxButtons::OK, MessageBoxIcon::Information);
		}
		return System::Void();
	}
	...
	String^ MainForm :: GetUIDFile() {
		String^ fileName = Environment::GetFolderPath(Environment::SpecialFolder::UserProfile) + "\\AppData\\LocalLow\\miHoYo\\原神\\UidInfo.txt";
		return fileName;
	}
	Void MainForm::SetCurrentUID() {
		this->btnSaveAccount->Text = "Saved Account(" + GetUID() + ")";
	}
	...
	Void MainForm::SaveAccount() {
		...
		if (exists) {
			auto  res= MessageBox::Show("Update Account("+GetUID()+")?", "Account Exist", MessageBoxButtons::YesNo, MessageBoxIcon::Information);
			if (res==System::Windows::Forms::DialogResult::Yes) {
				File::Delete(uidRegFile);
			}
			else {
				return;
			}
		...
		String^ cmd = "reg export HKCU\\Software\\miHoYo\\原神 " + uidRegFile;
		CmdCommit(cmd);
		if(!exists)SetDropDownBoxItems();
		MessageBox::Show("Save Account successfully!", "Info", MessageBoxButtons::OK, MessageBoxIcon::Information);
	}
	Void MainForm::SaveAccount(bool quiet) {
		String^ uid = GetUID();
		if (uid == "uid")return;
		if (!IsRegExists())return;
		String^ uidRegFile = GetDataDir() + "\\" + GetUID() + ".reg";
		...
		CmdCommit(cmd);
		if (!exists)SetDropDownBoxItems();
		if(!quiet)MessageBox::Show("Save Account successfully!", "Info", MessageBoxButtons::OK, MessageBoxIcon::Information);
	}
	BOOL MainForm::IsRegExists() {
		...
		rk = Registry::CurrentUser->OpenSubKey("Software\\miHoYo\\原神", true);
	...
	Void MainForm::doQuickSwitch()
	{
		if ((this->DropDownBox->SelectedIndex + 1) < this->DropDownBox->Items->Count)
		{
			...
			MessageBox::Show("Auto Switch completely!", "Info", MessageBoxButtons::OK, MessageBoxIcon::Information);
		}
